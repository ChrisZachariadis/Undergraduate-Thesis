...
import Frame from '../components/Frame'; // Ensure this component exists or replace with a suitable container
import SegmentedControl from '@react-native-segmented-control/segmented-control';
import AsyncStorage from '@react-native-async-storage/async-storage';
import moment from 'moment';
import { FontAwesomeIcon } from '@fortawesome/react-native-fontawesome';
import { faArrowLeft, faArrowRight } from '@fortawesome/free-solid-svg-icons';
import { styles } from './style'; // We'll define styles later
...
const KcalDetails = () => {
    ...
    // Fetch Kcal data from AsyncStorage on component mount and when relevant state changes
    useEffect(() => {
        fetchKcalData();
    }, [currentWeekStart, currentMonthStart, selectedIndex, currentDay]);

    // Function to fetch Kcal data
    const fetchKcalData = async () => {
        try {
            const storedData = await AsyncStorage.getItem('@garminData'); // Adjust the key if different
    ...
    const processDailyData = (data, day) => {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            hourData.push({
                value: hourEntry ? hourEntry.data.bmrKilocalories : 0,
                label: hourLabel,
                frontColor: hourEntry ? getBarColor(hourEntry.data.bmrKilocalories) : 'lightgrey',
            });
        }
    ...
    const processWeeklyData = (data, weekStart) => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            weekDays.push({
                value: dayData ? dayData.data.bmrKilocalories : 0,
                label: dayName,
                frontColor: dayData ? getBarColor(dayData.data.bmrKilocalories) : 'lightgrey',
            });
        }
    ...
    const processMonthlyData = (data, monthStart) => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            monthData.push({
                value: dayEntry ? dayEntry.data.bmrKilocalories : 0,
                label: day.format('D'), // Day number, e.g., '1', '2', ..., '31'
                frontColor: dayEntry ? getBarColor(dayEntry.data.bmrKilocalories) : 'lightgrey',
            });
        }
    ...
    // Function to determine bar color based on Kcal value
    const getBarColor = (kcal) => {
        if (kcal <= 1200) return 'grey'; // Example threshold
        if (kcal <= 1800) return '#FFA500'; // Orange for moderate
        return '#FF4500'; // OrangeRed for high
    };
    ...
    return (
        <Frame style={styles.container}>
            <View style={styles.headerContainer}>
                ...
                <Text style={styles.title}>Kilocalories Summary</Text>
            ...
            <SegmentedControl
                ...
                tintColor="#FFA500" // Matching the kcal theme
                activeTextColor="#fff"
            ...
            <View style={styles.graphContainer}>
                {chartData.length > 0 ? (
                                ...
                                Alert.alert(
                                    selectedIndex === 0
                                        ...
                                        : selectedIndex === 1
                                            ? `${item.label}`
                                            : `Day ${item.label}`,
                                    `BMR Kilocalories: ${item.value} kcal`,
                                    [{ text: 'OK' }],
                                    { cancelable: true }
                                );
                            }}
                    ...
                    <View style={styles.noDataContainer}>
                        <Text style={styles.noDataText}>No kilocalories data available for this period.</Text>
                    </View>
                )}
            </View>
        </Frame>
